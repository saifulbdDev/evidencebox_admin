...
import AddNewsNotify from '@/components/authority/AddNewsNotify';
import UpdateNewsNotify from '@/components/authority/UpdateNewsNotify';
...
} from '@/features/newsNotify/newsNotifyApi';
import { NewsNotify, NewsNotifyState } from '@/types/department';
...
const NewsNotify: React.FC = () => {
  const [isAdd, setAdd] = useState<boolean>(false);
  //@ts-ignore
  const [item, setItem] = useState<NewsNotify>({});

  const [isEdit, setIsEdit] = useState<boolean>(false);
  const [isDeleteModad, setIsDeleteModal] = useState<boolean>(false);
  const [deleteNewsNotify, { isLoading: isDelete }] = useDeleteNewsNotifyMutation();
  const { data, isLoading } = useGetNewsNotifyQuery('');
  const { newsNotifys } = useSelector((state: NewsNotifyState) => state.newsNotify);

  const columnHelper = createColumnHelper<NewsNotify>();
  const onDelatePlan = async () => {
    try {
      //@ts-ignore

      const response = await deleteNewsNotify(item);
  ...
  return (
    <div>
      <div className="mb-5 flex items-center justify-between">
        <div>
          <h2 className="text-xl mb-1">News notify</h2>
          <Breadcrumbs pages={breadcrumbs?.authorityNewsToNotify?.lists} />
        </div>
        <div>
          <Button
            onClick={() => setAdd(true)}
          ...
          >
            <PlusIcon className="h-5 w-5 text-white mr-1  hover:text-primary-400" /> Add news notify{' '}
          </Button>
        </div>
      </div>
      <Table isLoading={isLoading} data={newsNotifys || []} columns={columns} title="User List" />
      <AddNewsNotify open={isAdd} onClose={() => setAdd(false)} />
      <UpdateNewsNotify updateData={item} open={isEdit} onClose={() => setIsEdit(false)} />
      <DeleteModal
        title="Delete news notify"
