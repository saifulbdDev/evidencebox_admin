...
import AddNewsEmail from '@/components/authority/AddNewsEmail';
import UpdateNewsEmail from '@/components/authority/UpdateNewsEmail';
...
} from '@/features/newsEmail/newsEmailApi';
...
import { NewsEmailState } from '@/types/department';
const NewsEmail: React.FC = () => {
  const [isAdd, setAdd] = useState<boolean>(false);
  //@ts-ignore
  const [item, setItem] = useState<NewsEmail>({});

  const [isEdit, setIsEdit] = useState<boolean>(false);
  const [isDeleteModad, setIsDeleteModal] = useState<boolean>(false);
  const [deleteNewsEmail, { isLoading: isDelete }] = useDeleteNewsEmailMutation();
  const { data, isLoading } = useGetNewsEmailQuery('');
  const { newsEmails } = useSelector((state: NewsEmailState) => state.newsEmail);

  const columnHelper = createColumnHelper<NewsEmail>();
  const onDelatePlan = async () => {
    try {
      //@ts-ignore

      const response = await deleteNewsEmail(item);
  ...
  return (
    <div>
      <div className="mb-5 flex items-center justify-between">
        <div>
          <h2 className="text-xl mb-1">News email</h2>
          <Breadcrumbs pages={breadcrumbs.newsAuthorityEmail.lists} />
        </div>
        <div>
          <Button
            onClick={() => setAdd(true)}
          ...
          >
            <PlusIcon className="h-5 w-5 text-white mr-1  hover:text-primary-400" /> Add News email
          </Button>
        </div>
      </div>
      <Table isLoading={isLoading} data={newsEmails || []} columns={columns} title="User List" />
      <AddNewsEmail open={isAdd} onClose={() => setAdd(false)} />
      <UpdateNewsEmail updateData={item} open={isEdit} onClose={() => setIsEdit(false)} />
      <DeleteModal
        title="Delete Notify"
