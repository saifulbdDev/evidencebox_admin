...
import AddRelationType from '@/components/authority/AddRelationType';
import UpdateRelationType from '@/components/authority/UpdateRelationType';
...
} from '@/features/relationtype/relationtypeApi';
...
import { RelationTypeState } from '@/types/department';
const RelationType: React.FC = () => {
  const [isAdd, setAdd] = useState<boolean>(false);
  //@ts-ignore
  const [item, setItem] = useState<RelationType>({});

  const [isEdit, setIsEdit] = useState<boolean>(false);
  const [isDeleteModad, setIsDeleteModal] = useState<boolean>(false);
  const [deleteRelationType, { isLoading: isDelete }] = useDeleteRelationTypeMutation();
  const { isLoading } = useGetRelationTypeQuery('');
  const { relationTypes } = useSelector((state: RelationTypeState) => state.relationtype);


  const columnHelper = createColumnHelper<RelationType>();
  const onDelatePlan = async () => {
    try {
      //@ts-ignore

      const response = await deleteRelationType(item);
  ...
  const columns = useMemo(
    () =>
      [
        columnHelper.accessor('relationName', {
          cell: (info) => <span>{info.getValue() ? info.getValue() : '-'}</span>,
          header: () => <span>Relation name</span>,
        }),
  ...
  return (
    <div>
      <div className="mb-5 flex items-center justify-between">
        <div>
          <h2 className="text-xl mb-1">Relation type</h2>
          <Breadcrumbs pages={breadcrumbs?.relationType?.lists} />
        </div>
        <div>
          <Button
            onClick={() => setAdd(true)}
          ...
          >
            <PlusIcon className="h-5 w-5 text-white mr-1  hover:text-primary-400" />
            Add Relation Type
          </Button>
        </div>
      </div>
      <Table isLoading={isLoading} data={relationTypes || []} columns={columns} title="User List" />
      <AddRelationType open={isAdd} onClose={() => setAdd(false)} />
      <UpdateRelationType updateData={item} open={isEdit} onClose={() => setIsEdit(false)} />
      <DeleteModal
        title="Delete Notify"
