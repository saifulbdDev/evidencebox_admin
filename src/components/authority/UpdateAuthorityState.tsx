...
import { useUpdateAuthorityStateMutation } from '@/features/authorityState/authorityStateApi';
import Input from '@/components/ui/Input';
import Button from '@/components/ui/Buttons/Button';
import { AuthorityState, AuthorityNotify } from '@/types/department';
type UpdateAuthorityStateProps = {
  open: boolean;
  onClose: () => void;
  updateData: AuthorityState;
};

const UpdateAuthorityState = ({ open, onClose, updateData }: UpdateAuthorityStateProps) => {
  const { data, isLoading: isData } = useGetAuthorityNotifyQuery('');
  const [updateAuthorityState, { isLoading, isSuccess, error, isError }] =
  ...
  const submitHandler = async (values: typeof updateData) => {
    try {
      //@ts-ignore
      delete values.step;
      const response = await updateAuthorityState(values);
...
export default UpdateAuthorityState;
