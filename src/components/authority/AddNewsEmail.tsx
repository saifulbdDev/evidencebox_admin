...
import { useAddNewsEmailMutation } from '@/features/newsEmail/newsEmailApi';
...
const NewsEmail = ({ open, onClose }: NewsEmailProps) => {
  const { data, isLoading: isData } = useGetNewsNotifyQuery('');
  const [addNewsEmail, { isLoading }] = useAddNewsEmailMutation();
  const initialValues = {
    email: '',
    phone: '',
    newsToNotify: '',
  ...
  const submitHandler = async (values: typeof initialValues) => {
    try {
      //@ts-ignore
      delete values.step;
      const response = await addNewsEmail(values);
  ...
  return (
    <Drawer className="max-w-md" title="Add News Email" open={open} onClose={onClose}>
      <Formik
          ...
          <Form className="">
            ...
            <footer className="fixed bottom-0  border-t border-solid w-full px-5 inset-x-px">
              <div className="flex  flex-shrink-0 justify-end px-4 space-x-3  py-4">
                ...
                <Button
                  disabled={isValidating || !addNewsEmailSchema.isValidSync(values)}
