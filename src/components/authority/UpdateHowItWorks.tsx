...
import { setHowItWorks } from '@/features/department/departmentSlice';
...
const AddAuthorityDepartment = ({ open, onClose, fromData }: AddAuthorityDepartmentProps) => {
  ...
  const submitHandler = async (values: typeof fromData) => {
    try {
      ...
      values.description.forEach((element: string, i: number) => {
        formData.append(`description[${i}]`, element);
      });

      const response = await axios.put('/how-it-works', formData);
  ...
  return (
    <Drawer className="max-w-md" title="Update how It Works" open={open} onClose={onClose}>
      <Formik
        ...
        {({ errors, isValidating, isSubmitting, handleBlur, touched, values }) => (
          <Form className="">
            ...
            <FieldArray name="description">
              {({ insert, remove, push }) => (
                <>
                  {values.description.map((_: any, index: any) => (
              ...
              <div className="px-6 py-10 mt-2 border rounded-lg btn-gradient-1">
                <label htmlFor="file-upload" className="text-center cursor-pointer">
                  ...
                  <input
                    ...
                    className="sr-only"
                  />
                  <p className="text-sm text-gray-900">Click here to upload Photo</p>
                </label>
              </div>
              {file?.name ? (
            ...
            <footer className=" bottom-0  border-t border-solid w-full px-5 inset-x-px">
              <div className="flex  flex-shrink-0 justify-end px-4 space-x-3  py-4">
                ...
                <Button
                  disabled={isValidating || !howItWorksSchema.isValidSync(values)}
