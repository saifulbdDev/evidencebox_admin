...
import { editRelatives } from '@/features/relatives/relativesSlice';
...
const UpdateRelatives = ({ open, onClose, updateData }: RelativesProps) => {
  ...
  const submitHandler = async (values: typeof updateData) => {
    ...
    try {
      ...
      formData.append('relativeId', values?._id);
      formData.append('firstName', values?.firstName);
      formData.append('lastName', values?.lastName);
      formData.append('phone', values?.phone);
      formData.append('email', values?.email);
      formData.append('relationType', values.relationType);
      const response = await axios.put(`/update-relative`, formData);
      if ('error' in response) {
        ...
        dispatch(editRelatives(response?.data?.data?.relative));
        //@ts-ignore
        setFile([]);
        setImagePreviewUrl('');
        Swal.fire({
  ...
  return (
    <Drawer className="max-w-md" title="Update Relatives" open={open} onClose={onClose}>
      <Formik
          ...
          <Form>
              ...
              >
                ...
                <div className="relative">
                  <img
                    ...
                    src={imagePreviewUrl ? imagePreviewUrl : values?.photo}
                  />
                </div>
            ...
            <footer className="  border-t border-solid w-full px-5 inset-x-px">
              <div className="flex justify-end flex-shrink-0 px-4 py-4 space-x-3">
                ...
                <Button
                  disabled={isValidating || !relativesSchema.isValidSync(values)}
...
export default UpdateRelatives;
