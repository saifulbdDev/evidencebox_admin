...
import { useUpdateNewsNotifyMutation } from '@/features/newsNotify/newsNotifyApi';
import Input from '@/components/ui/Input';
import Button from '@/components/ui/Buttons/Button';

type UpdateNewsNotifyProps = {
  open: boolean;
  onClose: () => void;
  updateData: NewsNotify;
};

const UpdateNewsNotify = ({ open, onClose, updateData }: UpdateNewsNotifyProps) => {
  const [updateNewsNotify, { isLoading, isSuccess, error, isError }] =
  ...
  const submitHandler = async (values: typeof updateData) => {
    try {
      //@ts-ignore
      delete values.step;
      const response = await updateNewsNotify(values);
  ...
  return (
    <Drawer className="max-w-md" title="Update news Notify" open={open} onClose={onClose}>
      <Formik
...
export default UpdateNewsNotify;
