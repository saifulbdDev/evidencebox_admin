...
import { useAddAuthorityNotifyMutation } from '@/features/department/departmentApi';
...
const AddAuthorityNotify = ({ open, onClose }: AddAuthorityNotifyProps) => {
  const [addAuthorityNotify, { isLoading, isSuccess, error, isError }] =
  ...
  const submitHandler = async (values: typeof initialValues) => {
    try {
      //@ts-ignore
      delete values.step;
      const response = await addAuthorityNotify(values);
